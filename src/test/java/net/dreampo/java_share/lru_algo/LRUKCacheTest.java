...
public class LRUKCacheTest {
    ...
    @BeforeEach
    void setUp() {
        cache = LRUKCache.createDefault(3); // 容量为3的默认缓存（K=2）
    }
    ...
    void testFactoryMethods() {
        // 测试不同的工厂方法
        LRUKCache<String, String> defaultCache = LRUKCache.createDefault(10);
        assertNotNull(defaultCache);
        assertEquals(10, defaultCache.capacity());

        LRUKCache<String, String> adaptiveCache = LRUKCache.createAdaptive(10);
        assertNotNull(adaptiveCache);

        LRUKCache<String, String> smartCache = LRUKCache.createSmart(10);
        assertNotNull(smartCache);

        LRUKCache<String, String> timeBasedCache = LRUKCache.createTimeBased(10);
    ...
    void testCustomKValueStrategy() {
        // 测试自定义K值策略
        LRUKCache<String, String> customCache = LRUKCache.create(5, context -> {
            // 根据键的长度决定K值
    ...
    void testErrorHandling() {
        ...
        assertThrows(IllegalArgumentException.class, () -> {
            new LRUKCache<String, String>(0); // 容量不能为0
        });

        assertThrows(IllegalArgumentException.class, () -> {
            new LRUKCache<String, String>(10, null); // K函数不能为null
        });
