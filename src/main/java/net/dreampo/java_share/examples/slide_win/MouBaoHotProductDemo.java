...
public class MouBaoHotProductDemo {
     ...
     * @param score     热度分数
     */
        record ProductEvent(String category, String productId, LocalDateTime time, double score) {

        @Override
            public String toString() {
                return String.format("Event[%s,%s,%s,%.1f]", category, productId,
    ...
    static class SlidingWindowState {
        ...
        void addEvent(ProductEvent event) {
            ...
            productScores.merge(event.productId, event.score, Double::sum);
            totalEvents++;
        }
        
        void removeEvent(ProductEvent event) {
            eventsInWindow.removeFirst();
            double currentScore = productScores.get(event.productId);
            if (currentScore - event.score <= 0.001) {
    ...
    public static List<WindowResult> analyzeWithSlidingWindow(
        ...
        System.out.printf("总窗口数：%d\n", windowCount);
        System.out.printf("添加操作次数：%d\n", addOperations);
        System.out.printf("移除操作次数：%d\n", removeOperations);
        System.out.printf("总操作次数：%d\n", addOperations + removeOperations);
        
        // 计算总事件数
        long totalEvents = eventsByCategory.values().stream()
            .mapToLong(List::size)
            .sum();
        System.out.printf("总事件数：%d\n", totalEvents);
        
        // 如果使用朴素方法，每个窗口都要处理所有事件
        long naiveOperations = windowCount * totalEvents;
        System.out.printf("朴素方法操作次数：%d\n", naiveOperations);
    ...
    public static Map<String, List<ProductEvent>> generateData() {
        ...
        categoryProducts.put("手机数码", new String[]{"iPhone 15", "小米14", "华为Mate60", "三星S24", "OPPO Find"});
        categoryProducts.put("美妆护肤", new String[]{"SK-II神仙水", "雅诗兰黛", "兰蔻小黑瓶", "资生堂", "科颜氏"});
        categoryProducts.put("家用电器", new String[]{"戴森吹风机", "美的空调", "格力电扇", "小米净化器", "飞利浦剃须刀"});
        categoryProducts.put("玩具游戏", new String[]{"乐高积木", "Switch游戏机", "PS5", "泡泡玛特", "万代高达"});
        ...
        for (Map.Entry<String, String[]> entry : categoryProducts.entrySet()) {
            ...
            for (int minute = 0; minute < 180; minute++) {
                ...
                for (int i = 0; i < eventCount; i++) {
                    ...
                    // 热度分在10-100之间
                    double score = 10 + random.nextDouble() * 90;
    ...
    private static void visualizeResults(List<WindowResult> results) {
        ...
        for (Map.Entry<String, Map<String, Integer>> categoryEntry : sortedAppearances.entrySet()) {
            ...
            categoryEntry.getValue().entrySet().stream()
                ...
                .limit(3)
                .forEach(e -> System.out.printf("  - %s: %d次\n", e.getKey(), e.getValue()));
        }
    }
    ...
    public static void main(String[] args) {
        System.out.println("=== 某宝热门商品基于时间的滑动窗口分析（优化版） ===\n");
        ...
        int totalEvents = eventsByCategory.values().stream()
            .mapToInt(List::size)
            .sum();
        System.out.printf("生成事件数据：共 %d 个品类，%d 个事件\n", 
            eventsByCategory.size(), totalEvents);
        
        for (Map.Entry<String, List<ProductEvent>> entry : eventsByCategory.entrySet()) {
            System.out.printf("  - %s: %d 个事件\n", entry.getKey(), entry.getValue().size());
        }
        ...
        // 算法说明
        System.out.println("\n💡 优化算法的关键改进：");
        System.out.println("1. ✅ 数据按品类分组，每个品类内按时间排序");
        System.out.println("2. ✅ 使用真正的滑动窗口，维护窗口状态");
        System.out.println("3. ✅ 窗口滑动时只处理移出和移入的事件（增量计算）");
        System.out.println("4. ✅ 避免重复扫描，大幅提升性能");
        System.out.println("5. ✅ 空间复杂度优化，只维护当前窗口内的数据");
        
        // 业务应用
        System.out.println("\n🎯 实际应用场景：");
        System.out.println("• 实时热榜：毫秒级更新各品类热销榜单");
        System.out.println("• 流式处理：适用于Flink/Spark Streaming等框架");
        System.out.println("• 大数据场景：处理海量事件流时性能优势明显");
        System.out.println("• 资源优化：降低CPU和内存消耗");
    }
}
